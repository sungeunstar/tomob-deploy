...
await fs.mkdir(out,{recursive:true});const cache=new Map(),errors=[],network=[],reports={commit:ref,fixture:'generated preview + commit-pinned native runtime'};
...
try{
 ...
 const page=await browser.newPage({viewport:{width:1440,height:960},deviceScaleFactor:1});page.on('pageerror',e=>errors.push(e.message));page.on('console',m=>{if(m.type()==='error')errors.push(m.text());});page.on('response',r=>{if(r.status()>=400)network.push({url:r.url(),status:r.status()});});
 ...
 reports.errors=[...new Set(errors)];reports.network=network;reports.final=await page.evaluate(()=>window.__auroraQA.report());
 reports.accepted=reports.native.nativeTest?.pass===true&&reports.routes.every(r=>r.pass)&&reports.sightlines.every(r=>r.pass)&&!reports.final.errors.length&&!reports.final.assets.errors.length;
 if(!reports.accepted)process.exitCode=1;
}catch(e){reports.fatal=e.stack;reports.errors=errors;reports.network=network;process.exitCode=1;}
