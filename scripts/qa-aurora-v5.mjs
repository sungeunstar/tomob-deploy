...
try{
 ...
 await page.locator('body > canvas').click({position:{x:500,y:400}});const before=await page.evaluate(()=>({p:{...ctx.player.pos},locked:document.pointerLockElement===ctx.renderer.domElement}));await page.keyboard.down('w');await page.waitForTimeout(2500);await page.keyboard.up('w');const after=await page.evaluate(()=>({p:{...ctx.player.pos},animation:window.__curAnim?.()}));reports.manual={before,after};
 const data=await page.evaluate(()=>{ctx.renderer.render(ctx.scene,ctx.camera);return ctx.renderer.domElement.toDataURL('image/png');});await fs.writeFile(output+'/native.png',Buffer.from(data.split(',')[1],'base64'));
 ...
 for(const [name,p]of cache){const f=await p.catch(()=>null);if(!f||f.status!==200)continue;const safe=decodeURIComponent(name);if(safe.includes('..'))continue;const destination=output+'/runtime/site/'+safe;await fs.mkdir(path.dirname(destination),{recursive:true});await fs.writeFile(destination,f.body);}
 await fs.mkdir(output+'/runtime',{recursive:true});await fs.writeFile(output+'/runtime/external-index.json',JSON.stringify(external,null,2));
