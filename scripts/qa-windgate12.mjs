...
try{
 ...
 page=await browser.newPage({viewport:{width:1280,height:800},deviceScaleFactor:1});page.setDefaultTimeout(120000);
 page.on('pageerror',e=>{errors.push(e.message);console.error('PAGE_ERROR',e.stack);});page.on('console',m=>{if(m.type()==='error'&&!m.text().includes('Failed to load resource')){errors.push(m.text());console.error('BROWSER_ERROR',m.text());}});page.on('response',r=>{if(r.status()>=400)requests.push({url:r.url(),status:r.status()});});
 const base='http://127.0.0.1:8787/tomob-deploy/sandbox-aurora-v12.html';
 async function ready(){await page.waitForFunction(()=>window.__auroraQA?.report().ready||document.getElementById('load-note')?.textContent.includes('초기화 실패'),null,{timeout:180000});const s=await page.evaluate(()=>({ready:window.__auroraQA?.report().ready,note:document.getElementById('load-note')?.textContent,errors:window.__auroraErrors}));if(!s.ready)throw new Error('BOOT: '+JSON.stringify(s));await page.evaluate(()=>ctx.setRenderOverride(()=>{}));}
