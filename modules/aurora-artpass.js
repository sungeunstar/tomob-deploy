...
export function sculptTerrain(field){
 ...
 for(let j=0;j<=N;j++)for(let i=0;i<=N;i++){
  ...
  // A carved bowl AND a raised bank, rather than placing water over an old slope.
  for(const b of BASINS){const r=basinRadius(b,x,z);if(r<1.6){const bowl=b.level-b.depth+(b.depth+1.35)*smooth(.2,1.19,r);h=mix(h,bowl,1-smooth(1.19,1.60,r));}}
...
export async function initAuroraArtpass(ctx,options={}){
 ...
 // Rich, quiet large-scale colour variation. Close detail comes from a tiled bump map.
 ...
 // Cliff outcrops are anchored into the slope; paths and door approaches stay clear.
 ...
 // A thin bridge with a continuous collider; no plank-gap catches.
 const bridgeY=x=>11.25-.65*Math.sin(clamp((x+64)/34)*Math.PI),bp=[],bi=[];
 for(let i=0;i<=68;i++){const x=-64+i*.5,y=bridgeY(x);bp.push(x,y,1.7,x,y,6.3);if(i<68){const k=i*2;bi.push(k,k+1,k+2,k+1,k+3,k+2);box(x+.23,y-.1,4,[.45,.2,4.7],i%5?wood:wood2);}}
 ...
 for(const z of[1.8,6.2])for(let i=0;i<=10;i++){const x=-64+i*3.4,y=bridgeY(x);beam([x,y,z],[x,y+1.15,z]);if(i<10)beam([x,y+1.03,z],[x+3.4,bridgeY(x+3.4)+1.03,z],.045);}
 // WATER: triangle-clipped water surfaces, not radial fans. Depth comes from terrain.
 ...
 for(const b of BASINS){const ps=[],ds=[],edge=[],step=SIZE/N;const x0=-SIZE/2+Math.floor((b.x-b.rx*1.5+SIZE/2)/step)*step,z0=-SIZE/2+Math.floor((b.z-b.rz*1.5+SIZE/2)/step)*step;
  for(let z=z0;z<b.z+b.rz*1.5;z+=step)for(let x=x0;x<b.x+b.rx*1.5;x+=step){const v=[[x,z],[x+step,z],[x,z+step],[x+step,z+step]].map(([x,z])=>({x,z,d:b.level-field.height(x,z)-.035}));if(basinRadius(b,x,z)>1.4)continue;for(const ids of[[0,2,1],[1,2,3]]){const poly=clipSpill(clipPoly(ids.map(i=>v[i])),b.spillZ);for(let i=1;i<poly.length-1;i++)for(const p of[poly[0],poly[i],poly[i+1]]){ps.push(p.x,b.level,p.z);ds.push(Math.max(0,p.d));if(p.d===0)edge.push(p);}}}
  ...
  const contains=(x,z)=>(b.spillZ==null||z<=b.spillZ)&&basinRadius(b,x,z)<1.4&&field.height(x,z)<b.level-.035;waterRegions.push({...b,contains});let maxGap=0;for(const p of edge)maxGap=Math.max(maxGap,Math.abs(b.level-field.height(p.x,p.z)-.035));basinQA.push({id:b.id,triangles:ps.length/9,bankEdgeError:+maxGap.toFixed(3)});
 }
 ...
 // Textured falling volume: many cross-stream vertices, broken edges and foam only at impact.
 ...
 // SUMMIT: segmented masonry instead of a stack of clean cubes.
 const py=46,cx=-9,cz=-41;
 for(let i=0;i<3;i++)mesh(new THREE.CylinderGeometry(10.7-i*.6,11-i*.6,.22,12),i===0?dark:stone,cx,py+.11+i*.20,cz,[1,1,1],true);
 ...
 // Existing buildings, scaled against the same unchanged native character.
 ...
 // Small, human-scale work yards. Collisions are simple, not decorative triangle soup.
 ...
 // Paths are bordered selectively, not continuous artificial rails everywhere.
 ...
 // Native wharf, same source/material mapping. A raycast chooses a real deck anchor.
 ...
 jobs.push(dock());
 // Native vegetation with denser understorey concentrated by region.
 async function prototype(path,fit='height'){const g=await gltf.loadAsync(url(path));g.scene.updateMatrixWorld(true);const bb=new THREE.Box3().setFromObject(g.scene),s=bb.getSize(new THREE.Vector3()),c=bb.getCenter(new THREE.Vector3()),scale=1/(fit==='height'?s.y:Math.max(s.x,s.y,s.z)),parts=[];g.scene.traverse(o=>{if(o.isMesh){const geo=o.geometry.clone();geo.applyMatrix4(o.matrixWorld);geo.translate(-c.x,-bb.min.y,-c.z);geo.scale(scale,scale,scale);G.add(geo);const mats=(Array.isArray(o.material)?o.material:[o.material]).map(m=>{const a=m.clone();M.add(a);for(const t of Object.values(a))if(t?.isTexture)T.add(t);return a;});parts.push({g:geo,m:Array.isArray(o.material)?mats:mats[0]});}});return parts;}
 const trees=[[],[],[],[]];for(let i=0;i<9000&&placements.length<225;i++){const x=(rng()-.5)*212,z=(rng()-.5)*188,y=field.height(x,z);if(y<7||field.slope(x,z)>.5||field.nearPath(x,z).d<5.5||excluded(x,z,3)||placements.some(p=>Math.hypot(x-p.x,z-p.z)<3.7))continue;const h=6+rng()*5.8,k=Math.floor(rng()*4),p={type:'tree',x,y,z,s:[h,h,h],rot:rng()*6.28};trees[k].push(p);placements.push(p);fixed(R.ColliderDesc.cylinder(h*.30,.32).setTranslation(x,y+h*.30,z));}
 ...
 // Read/query interfaces stay compatible with the original terrain module.
 const ray=new THREE.Raycaster(),origin=new THREE.Vector3(),down=new THREE.Vector3(0,-1,0);ray.firstHitOnly=true;
 const api={root,field,collide,assets,portal,waterRegions,basinQA,placementPoints:placements,spawn:{x:-43,y:field.height(-43,66)+2.3,z:66},radius:145,offset:{x:0,z:0},data:{name:'바람의 차원문 섬 · 05',objs:[]},_allObjs:[root],qualityVersion:'aurora-v5-artpass',detailStats:{grass:grassItems.length,ferns:groundMoss.length,bankRocks:bankRocks.length,cliffs:cliffs.length,ruinFragments:ruinBits.length,pathStones:pathStones.length,props:props.length},addTrimesh:solid,
