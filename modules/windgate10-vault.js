/** Windgate 10: actual cut-through vault, not a decorative shell on uncut terrain.
...
const controls=[[-18,-68,55],[-11,-73,55.3],[-5,-79,55.7],[-5.7,-85,56.2],[-10,-91,58],[-13,-98,61]];
...
function vaultSDF(x,y,z){const q=vaultSample(x,z),r=3.5;return Math.max(q.d-r,q.y-.38-y,y-(q.y+2.2+3.05*Math.sqrt(Math.max(0,1-(q.d/r)**2))));}
export function prepareVault(field){
 field.vaultReserve=vaultReserve;
 field.vaultRows=rows;
 const points=rows.map(r=>[r.p.x,r.p.z,r.p.y]);
 for(let i=0;i<points.length-1;i++)field.segmentsList.push({a:points[i],b:points[i+1],id:'vault',width:6.4,bridge:true});
 field.routeDefs.push({id:'vault',name:'성소 아래 굽은 순례굴',width:6.4,points});
 const stations=[['vault-entry','옛 순례굴 입구',0],['vault-turn','굴 안쪽 굽이',.40],['vault-exit','순례굴 출구',1]];
...
export function carveVaultTerrain(geo,field){
 ...
 for(let i=0;i<ix.length;i+=3){const t=[read(ix[i]),read(ix[i+1]),read(ix[i+2])],x=(t[0][0]+t[1][0]+t[2][0])/3,z=(t[0][2]+t[1][2]+t[2][2])/3;
  if(x>-25&&x<5&&z>-105&&z<-61&&vaultSample(x,z).d<6){refined++;split(t,2);}else emit(t);}
...
export function dressWindgate10(ctx,island){
 const field=island.field,root=new THREE.Group();root.name='Windgate 10 / undercroft and route finish';island.root.add(root);
 ...
 const rock=mat('#697367',{vertexColors:true,side:THREE.DoubleSide}),masonry=mat('#8e9481'),dark=mat('#505b50'),wood=mat('#67513a'),iron=mat('#45483f',{metalness:.3});
 ...
 for(let i=0;i<rows.length;i++){const r=rows[i];for(let j=0;j<=10;j++){const l=(j/10*2-1)*width,edge=Math.abs(l)/width;pos.push(...toWorld(r,l,-.01+edge*edge*.06));const shade=.58+.3*Math.abs(r.u-.5)*2;colors.push(shade,shade,shade);uv.push(j/10,r.u*18);if(i<rows.length-1&&j<10){const k=i*11+j;ix.push(k,k+11,k+1,k+1,k+11,k+12);}}}
 ...
 const inside=[];for(let i=0;i<wi.length;i+=3){const a=wi.slice(i,i+3),x=a.reduce((s,k)=>s+wp[k*3],0)/3,y=a.reduce((s,k)=>s+wp[k*3+1],0)/3,z=a.reduce((s,k)=>s+wp[k*3+2],0)/3;if(field.height(x,z)>y-.05)inside.push(...a);}
 ...
 for(let i=0;i<75;i++){const r=rows[10+Math.floor(rnd()*100)],side=rnd()<.5?-1:1,p=toWorld(r,side*(2.92+rnd()*.25),.06);add(stoneG,i%3?dark:masonry,p,[.12+rnd()*.2,.1,.19+rnd()*.18],[0,rnd()*6.28,0],false);}
 ...
 sign(-53,29,[['광산',[-58,11]],['성소',[-44,22]]]);sign(36,5,[['망루',[62,-6]],['해변',[55,14]]]);sign(-103,-3,[['광산',[-107,-18]],['항구',[-102,11]]]);sign(-22,-66,[['옛 순례굴',[-11,-73]],['옛길',[-25,-83]]]);
 ...
 island.vault={rows:rows.map(r=>r.p.toArray()),sample:vaultSample,cut:field.vaultCut,length:curve.getLength(),signs,shoulders,root,wallMeshes,get cameraCorrections(){return cameraCorrections;}};
 return island.vault;
}
