...
`);};m.customProgramCacheKey=()=> 'windgate07-integrated-strata';m.needsUpdate=true;}
 ...
 for(let i=0;i<8000&&placed.length<132;i++){
  ...
  if(island.field.vaultReserve(x,z,4)||y<8||sl<1.1||field.nearPath(x,z).d<11||BUILDINGS.some(b=>Math.hypot(x-b.x,z-b.z)<b.r+6)||views.some(v=>axisDistance(v,x,z).d<8)||BASINS.some(b=>basinRadius(b,x,z)<1.03)||fallAxes.some(a=>axisDistance(a,x,z).d<a[6]*.5+3)||placed.some(p=>Math.hypot(p.x-x,p.z-z)<5))continue;
